...
>
	...
	<button
		...
		class="min-w-[100px] max-md:hidden"
		>{locales.about[$page.params.language as 'en' | 'ka']}</button
	>
	<button on:click={() => goto(`/${$page.params.language}/blog`)} class="max-md:hidden"
		>{locales.blog[$page.params.language as 'en' | 'ka']}</button
	>
	<button on:click={scroll} class="min-w-[100px] max-md:hidden"
		>{locales.contact[$page.params.language as 'en' | 'ka']}</button
	>
